...
const bisectDate = bisector<ResultActiveAddresses, Date>(
  (d) => new Date(d.date)
).left;

interface ActiveAddressesPerDayProps extends WithParentSizeProps {
  statsData: ResultActiveAddresses[];
}

const ActiveAddressesPerDay = ({
  statsData,
  parentHeight: height,
  parentWidth: width,
}: ActiveAddressesPerDayProps & WithParentSizeProvidedProps) => {
  // Remove first element as it's breaking the chart
  ...
  const {
    ...
    showTooltip,
    tooltipData,
  } = useTooltip<ResultActiveAddresses>();
      ...
      {tooltipOpen && tooltipData && (
        ...
        >
          <Tooltip>
            ...
            <TooltipText>
              Sender count: {tooltipData.senderUniqueAddresses}
            </TooltipText>
            <TooltipText>
              Receiver count: {tooltipData.recipientUniqueAddresses}
            </TooltipText>
          </Tooltip>
        </TooltipInPortal>
      )}
...
const enhancedActiveAddressesPerDay =
  withParentSize<ActiveAddressesPerDayProps>(ActiveAddressesPerDay);

export { enhancedActiveAddressesPerDay as ActiveAddressesPerDay };
